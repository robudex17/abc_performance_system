...
export const useManagerStore2 = defineStore('managers2', () => {
   //call auth store fetch the token on the localstorage 
    ...
    const fetchManagers = async (departmentCode) => {
        state.loading = true 
        state.error = null

        try {
            const response = await fetch(`${API.fetchManagers2}/${departmentCode}`,{
                method: 'GET',
                headers: {
