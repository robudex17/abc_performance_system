...
export const useTeamStore2 = defineStore('teams2', () => {
   //call auth store fetch the token on the localstorage 
    ...
    const fetchTeams = async (teamId, departmentCode, queryString) => {
        state.loading = true 
        state.error = null
        let url = API.teams2
        ...
        if(teamId) {
            url = `${API.teams2}/${departmentCode}/${teamId}`
        }else{
            url = `${API.teams2}/${departmentCode}`
        }   
       ...
       const addUpdateDeleteTeam = async (data, httpMethod, sucessMessage) => {
        ...
        try {
            const response = await fetch(`${API.teams2}`, {
                method: httpMethod,
           ...
           await fetchTeams(null, data.department_code)
           alert(sucessMessage)
        } catch (error) {
